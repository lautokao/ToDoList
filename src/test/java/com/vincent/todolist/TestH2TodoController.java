...
@Sql(scripts = "classpath:test/data.sql",
        config = @SqlConfig(encoding = "utf-8", transactionMode = SqlConfig.TransactionMode.ISOLATED)) // sql 檔案放置的地方
@DirtiesContext(classMode = DirtiesContext.ClassMode.BEFORE_EACH_TEST_METHOD) // 在類中別的每個測試方法之前
public class TestH2TodoController {
    @Autowired
    ...
    public void testCreateTodos() throws Exception {
        ...
        // [Assert] 判定回傳的body是否跟預期的一樣
        assertEquals(5, Integer.parseInt(actualId));
    }
